...
|---|---|---|---|
| `starRating` | `"A" \| "B" \| "C" \| "TODOS"` | Sim | Tier do catálogo — `TODOS` = aplica a qualquer lead |
...
Existem **4 cards**, um por tier. Os três primeiros são exclusivos por nível de estrela. O quarto (`TODOS`) é enviado a **qualquer lead** independente da estrela — a IA sempre inclui as entradas `TODOS` junto com o tier específico do lead.
...
┌──────────────┐  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐
│  Tier C  ⭐  │  │ Tier B  ⭐⭐  │  │ Tier A ⭐⭐⭐  │  │    TODOS     │
│  Lead 1★    │  │  Lead 2★★   │  │  Lead 3★★★  │  │ Todos leads  │
│──────────────│  │──────────────│  │──────────────│  │──────────────│
│ 1 – 50 un   │  │ 1 – 50 un   │  │ 1 – 50 un   │  │ 1 – 50 un   │
│ R$ 54,90/un │  │ R$ 49,90/un │  │ R$ 44,90/un │  │ R$ 59,90/un │
│ desc. 0%    │  │ desc. 5%    │  │ desc. 10%   │  │ desc. 0%    │
│[···][✏][🗑] │  │[···][✏][🗑] │  │[···][✏][🗑] │  │[···][✏][🗑] │
│──────────────│  │──────────────│  │──────────────│  │──────────────│
│ 51 – 200 un │  │ 51 – 200 un │  │ 51 – 200 un │  │ 51 – 200 un │
│ R$ 52,00/un │  │ R$ 46,00/un │  │ R$ 41,00/un │  │ R$ 56,00/un │
│ desc. 0%    │  │ desc. 8%    │  │ desc. 15%   │  │ desc. 0%    │
│[···][✏][🗑] │  │[···][✏][🗑] │  │[···][✏][🗑] │  │[···][✏][🗑] │
└──────────────┘  └──────────────┘  └──────────────┘  └──────────────┘
```

> **Importante para o dev:** a IA busca as faixas `WHERE starRating = tier_do_lead OR starRating = 'TODOS'`. Ou seja, o lead recebe as faixas do seu tier **e** todas as faixas marcadas como `TODOS` — ambas são enviadas juntas na mesma mensagem.
...
| Valor enviado | Label exibido | Quando usar |
|---|---|---|
| `C` | ⭐ Tier C — Leads com 1 estrela | Preço para leads até R$ 999 |
| `B` | ⭐⭐ Tier B — Leads com 2 estrelas | Preço para leads de R$ 1.000–R$ 4.999 |
| `A` | ⭐⭐⭐ Tier A — Leads com 3 estrelas | Preço para leads com R$ 5.000+ |
| `TODOS` | 👥 Todos os leads | Faixa enviada a qualquer lead, independente da estrela |
...
| Card `TODOS` com faixas | Exibir badge neutro `"Enviado a todos os leads"` no header |
...
3. Formata e envia a mensagem com a tabela de preços + mídias do tier.  
   A query do backend é: `starRating = tier_do_lead OR starRating = 'TODOS'`  
   → O lead recebe **as faixas do seu tier + todas as faixas TODOS** juntas.
4. O frontend deve exibir em cada card de tier o badge informativo:
   ...
   - TODOS → `"Enviado a todos os leads — complementa o tier específico"`
...
|---|---|
| `PriceTableTab` | Container da aba com os **4 cards** de tier (C, B, A, TODOS) |
| `PriceTierCard` | Card vertical de um tier — aceita `"A" \| "B" \| "C" \| "TODOS"` como prop |
| `PriceEntryRow` | Linha de faixa dentro do card com ações de editar/excluir/detalhar |
| `PriceEntryModal` | Modal de criação e edição — select com as 4 opções de tier |
